...
def format_text_on_node(threat_name: str, monetary_amount: float, prob: float) -> str:
    """
    Format the text on a node describing the security threat it represents.
    ...
    # The formatted text on each node will display the following information:
...
def get_list_of_probs_and_monet_amounts_from_children_dict(
    ...
    """
    Get the list of probabilities and monetary amounts from a list of dictionaries of children nodes of an attack tree.

    Args:
        children_nodes_list_of_dicts: List[dict]
                                    A list of dictionaries pertaining to the children nodes of an attack tree.

    Returns:
            A tuple with two lists of floats, the first one with a list of monetary amounts and the second one with a
    ...
    # The probabilities of the leaf nodes are assumed to be from independent events and, thus, multiplied
    # to compute the probability at their parent node based on the independent events downstream/at its children nodes.
    # The monetary amount at the parent node is the sum of the monetary amounts of the security threats at its
    # children nodes.
